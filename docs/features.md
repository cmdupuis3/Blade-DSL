...
| `segments(A)` / `files(A)` / `segments(C0, C1)` | structural GROUP BY | Groupings with no key array and no permutation; `files` is the labelled file level of a tiled axis; the two-alias form is the tile grouping of a rank-2 array in slot order. Statically evaluable for literal grids. Over a rank-1 `.stream` variable, `group_by` reads one run at a time (zarr) |

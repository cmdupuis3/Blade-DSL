...
| 3 | #2 P1a's soundness half: literal window offsets outside the declared REACH refused (BL4019), the carousel tail guarded; the shared `HaloAccess` record as their one source | refusal and guard **landed** (`tests/corpus/loops/202`, `OptimizeTests.halo_carousel_tail_guarded`); the shared record itself is P1a's remaining work -- the two fixes read the tag through the existing `HaloWinTag` parser, which is the seam the record replaces |
...
| 5 | #2 P1b: the backward gather for halo cotangents (routes S and G, bitwise gate) | queued |

...
| 6a | #7 segmented domains: the structural `group_keys` case, `ungroup`, per-segment reads, the file-label `EnumIdx`, the runtime coordinate obligation, licensed per-segment folds, slot-aware multi-dim grouping | **built 2026-09-06** on feat/segmented-domains (5 commits): `Chunked` in all three forms, `segments`/`files`/`segments(C0, C1)`, `ungroup` (grouped, row and tile forms), static `segments`, zarr inherited edges, per-segment streamed reads (zarr rank 1), `blocked` deleted (`tests/corpus/segments` 13, zarr lane 10c-10e, sql.md §7c); open: halo ghosts, nested two-level shape, 2-D mosaic declaration, netcdf/icechunk edges |

...
| 3 | Streaming across reduction boundaries | **DEFER** the compiler feature; **GO** on an idiom package + two defect fixes + one typecheck rewrite -- ALL FOUR LANDED (A + B in `4a72673`; C = the join share read by a direct-fold leg and D = the partial-fold rewrite of the deferred outer product, r = 2, on 2026-09-07) | S-M each | No | [03-streaming-reductions.md](structural/03-streaming-reductions.md) |
...
| 5 | Structured operators (Gram application without a Gram) | **GO** for one applied-action node -- STEP 1 BUILT 2026-09-07 (`gram_apply` node, twins, transposed-gemv route, AD rules; feat/streaming-reductions-cd); operator values / factorizations deferred to a co-design with fortran-killer-2 6.2 | 3.5-6 days, ~30 files | **Yes: seven** (spelling, home, sugar, complex convention, value-form verb, loop order, advisory) | [05-structured-operators.md](structural/05-structured-operators.md) |
...
| 7 | Segmented domains: chunks and files as a structural `group_by` of one axis (reframes #4; the serial half of plan-distributed-memory P0/P0.5) | **GO**, ahead of #4 | ~6.5 weeks over 10 pieces | **Resolved 2026-09-06** except D6 (unlicensed-fold policy): `Chunked<I, [[s, f], ...]>`, static `segments(I)`, `EnumIdx` labels, `ungroup` with `join` untouched, provider-agnostic multi-store, `blocked` deleted, slot order, coordinates never consulted, fixed interleaved order with a refusal | [07-segmented-domains.md](structural/07-segmented-domains.md) |
...
| 6 | #4 revision reuse, elementwise first, with the scale run as a real stop condition | re-based on #7 (structural/07): the reuse unit becomes the segment; queued after it |
| 6a | #7 segmented domains: the structural `group_keys` case, `ungroup`, per-segment reads, the file-label `EnumIdx`, the runtime coordinate obligation, licensed per-segment folds, slot-aware multi-dim grouping | **built 2026-09-06** on feat/segmented-domains (7 commits): `Chunked` in all three forms, `segments`/`files`/`segments(C0, C1)`/`segments(a)`, `ungroup` (grouped, row and tile forms), static `segments`, zarr inherited edges, streamed consumers (fold in storage order, per-run/per-tile group_by, elementwise by block or row band, stencil with ghost cells; rank 1 and 2) recorded as `segment-streaming` for `blade plan`, `blocked` deleted (`tests/corpus/segments` 15, zarr lane 10c-10i, sql.md §7c); open: nested two-level shape, 2-D mosaic declaration, a cost model for the streaming choice, netcdf/icechunk edges, rank >= 3 |

...
| 6a | #7 segmented domains: the structural `group_keys` case, `ungroup`, per-segment reads, the file-label `EnumIdx`, the runtime coordinate obligation, licensed per-segment folds, slot-aware multi-dim grouping | **partly built 2026-09-06** on feat/segmented-domains: `Chunked` in all three forms, `segments`/`files`, `ungroup` (grouped + row forms), static `segments`, zarr inherited edges, `blocked` deleted (`tests/corpus/segments`, zarr lane 10c/10d, sql.md §7c); not built: per-segment lazy reads, slot-aware multi-dim group_by + cross-slot refusal, halo ghosts, licensed per-segment folds (D6 open) |
